...
class AnonymityBreaker:
    ...
    def differential_analysis(self):
        ...
        for v_id in versions:  # 遍历每个版本的ID
            ...
            rows, cols = data.shape  # 40行 x 50列
    ...
    def track_reconstruction(self):
        ...
        #!  新增：聚类 eps 动态绑定到最大步长
        eps_val = max(self.data_client.track_max_step.values()) if self.data_client.track_max_step else 0.2  # 动态绑定到最大步长，默认0.2
        clustering = DBSCAN(eps=eps_val, min_samples=5).fit(coords)  # 对坐标进行DBSCAN聚类，eps为最大步长，min_samples为最小样本数
        df['cluster'] = clustering.labels_  # 每个点的聚类标签，-1表示噪声点
        
        reconstructed_tracks = {}  # 存储所有重建轨迹的字典
        unique_clusters = set(df['cluster'])  # 获取所有唯一的聚类标签(set无序不重复)
        if -1 in unique_clusters: unique_clusters.remove(-1)  # 移除噪声点（标签为-1的类）
        
        max_cluster_size = max(len(df[df['cluster'] == cid]) for cid in unique_clusters) if unique_clusters else 1  # 计算最大簇的大小，默认1
        versions_count = len(self.dataset_versions)  # 数据集版本数量
        print(f"    Identified {len(unique_clusters)} distinct vessel tracks.")

        for cluster_id in unique_clusters:  # 遍历每个唯一的聚类标签
            track_points = df[df['cluster'] == cluster_id].sort_values('time_idx')  # 布尔索引+排序，获取当前簇的所有点，按时间索引升序排列
            processed_track = []  # 存储当前簇的重建轨迹点列表

            for _, row in track_points.iterrows():  # 遍历当前轨迹的每个点
                node_conf = 0.5 + 0.5 * float(min(row['val'] / 5.0, 1.0))  # 计算节点的置信度
                # 构建轨迹点信息
                processed_track.append({'lat': row['lat'], 'lon': row['lon'], 'time_seq': row['time_idx'], 'confidence': node_conf})
            
            reconstructed_tracks[f"Target_{cluster_id}"] = processed_track  # 将轨迹存入字典
            start, end = processed_track[0], processed_track[-1]  # 获取轨迹的起点和终点
            print(f"    [Target_{cluster_id}] Path: ({start['lat']},{start['lon']}) -> ... -> ({end['lat']},{end['lon']})")
           
            size_score = len(track_points) / max_cluster_size   # 计算规模得分
            strength_score = float(np.clip(track_points['val'].mean() / 5.0, 0.0, 1.0))  # 计算强度得分
            temporal_score = track_points['time_idx'].nunique() / max(1, versions_count)  # 计算时间跨度得分
            total_score = round(0.4 * size_score + 0.3 * strength_score + 0.3 * temporal_score, 3)  # 计算总分
            self.track_scores[f"Target_{cluster_id}"] = {
                'size': round(size_score,3), 
                'strength': round(strength_score,3), 
                'temporal': round(temporal_score,3), 
                'score': total_score}  # 存储轨迹评分
    ...
    def visualize_results(self, tracks):
        ...
        try:
            ...
            colors = ['#00ff00', '#00ffff', '#ff00ff', '#ffff00', '#ff0000', '#0000ff', '#00ff00', '#800080', '#808000', '#008000', '#808080'] # 鲜艳的颜色方便观察
