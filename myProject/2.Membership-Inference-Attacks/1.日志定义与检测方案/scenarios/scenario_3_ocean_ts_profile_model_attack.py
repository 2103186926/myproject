...
# scenario_3_ocean_ts_profile_model_attack.py
...
def main():
    print(f"[+] Starting Membership Inference Attack for model 'ocean_ts_profile_model'")
    
    # 阶段1: 准备敏感数据和影子数据
    global SENSITIVE_SAMPLE
    SENSITIVE_SAMPLE = np.array([0.5345085948377027, 1967.6516355693484, 12.773532502951909, 0.3487418059006198, 2.3939586721351525])
    
    print(f"[+] Generating 145 shadow samples...")
    shadow_samples = [SENSITIVE_SAMPLE + np.random.normal(0, 0.05, SENSITIVE_SAMPLE.shape) for _ in range(145)]
    
    all_samples = [SENSITIVE_SAMPLE] + shadow_samples
    
    # 阶段2: 查询目标模型
    print(f"[+] Querying the target model for {len(all_samples)} samples...")
    model_client = PredictiveModelClient(model_name="ocean_ts_profile_model")
    model_responses = []
