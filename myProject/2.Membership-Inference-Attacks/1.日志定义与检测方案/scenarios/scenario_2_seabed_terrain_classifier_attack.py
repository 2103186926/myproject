...
# scenario_2_seabed_terrain_classifier_attack.py
...
def main():
    print(f"[+] Starting Membership Inference Attack for model 'seabed_terrain_classifier'")
    
    # 阶段1: 准备敏感数据和影子数据
    global SENSITIVE_SAMPLE
    SENSITIVE_SAMPLE = np.array([0.17835769913323807, 268.34523142430413, 7.4617102788009175, 0.03512960338154669, 3.449213702555174, 4.402258961237853, 0.2282687587759391, 1.4187281841454935])
    
    print(f"[+] Generating 250 shadow samples...")
    shadow_samples = [SENSITIVE_SAMPLE + np.random.normal(0, 0.05, SENSITIVE_SAMPLE.shape) for _ in range(250)]
    
    all_samples = [SENSITIVE_SAMPLE] + shadow_samples
    
    # 阶段2: 查询目标模型
    print(f"[+] Querying the target model for {len(all_samples)} samples...")
    model_client = PredictiveModelClient(model_name="seabed_terrain_classifier")
    model_responses = []
