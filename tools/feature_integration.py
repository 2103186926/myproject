...
def process_file_pair(semantic_feature_file: str, 
    ...
    if output_file is None:
        ...
        output_file = f"{semantic_base}.npy"
...
def process_directory_pair(semantic_dir: str, 
    ...
    if pattern_match:
        ...
        for s_file in semantic_files:
            ...
            # 尝试在行为特征目录中找到同名文件
            b_file = s_base + ".npy"
            if b_file in behavior_files:
                file_pairs.append((
                    os.path.join(semantic_dir, s_file),
                    os.path.join(behavior_dir, b_file),
                    os.path.join(output_dir, f"{s_base}.npy")  # 保持与输入文件同名
                ))
                logger.info(f"匹配到同名文件: {s_file} 和 {b_file}")
            else:
                logger.warning(f"未找到与 {s_file} 同名的行为特征文件")
    else:
        # 交叉匹配所有文件（笛卡尔积）
        for s_file in semantic_files:
